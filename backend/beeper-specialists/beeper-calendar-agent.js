...
class BeeperCalendarAgent {
  ...
  async initialize() {
    try {
      logger.info('Initializing Beeper Calendar Agent...');
      ...
      logger.info('Beeper Calendar Agent initialized successfully');
      return true;
    } catch (error) {
      logger.error('Failed to initialize Beeper Calendar Agent', { error: error.message });
      throw error;
    }
  }
  ...
  async extractEvent(message) {
    try {
      logger.info(`Extracting calendar event from ${message.platform} message: ${message.from_name}`);
  ...
  buildCalendarPrompt(message) {
    return `You are a specialized calendar event extraction AI for chat messages. Your task is to analyze a message from ${message.platform} and extract detailed calendar event information.
...
Your task is to:
1. Determine if this message contains a calendar event (meeting, appointment, hangout, "let's meet", etc.)
...
- **Organizer**: Who is organizing (name)

IMPORTANT:
- If a field cannot be determined, set it to null
- Be careful with date/time parsing - consider context like "tomorrow", "next week", "Friday"
- Look for common patterns: "let's meet", "see you at", "appointment", "hangout"
...
- For casual meetups like "coffee tomorrow at 2", still extract as events
...
Return ONLY a valid JSON object with no additional text. Format:
{
  ...
  "event": {
    ...
    "meetingLink": null,
    "description": "Casual coffee catch-up",
    "organizer": "John",
    "confidenceScores": {
      "name": 0.90,
      "date": 0.95,
...
If no event is found:
{
  ...
  async processEventMessages() {
    try {
      logger.info('Processing Beeper event messages...');
      ...
      const messages = await this.db.all(`
        ...
        LEFT JOIN beeper_events e ON m.id = e.message_id
        ...
        try {
          ...
          if (result.hasEvent && result.event) {
            ...
            await this.db.insertEvent({
              messageId: message.id,
              title: result.event.name || `Event from ${message.from_name}`,
...
module.exports = BeeperCalendarAgent;
